...
class add extends Controller
{
	...
	public function add(){
		...
		if($add_type=='1'){
			...
			echo '餐饮娱乐';
			return view('add_cyyl',['add_type'=>$add_type,'create_type'=>$create_type,'signPackage'=>$signPackage]);
...
//						$pic  .=  $info->getFilename().',';
			...
			$res = Db::table('i_vote_list')->where('id',$id)->update(['uid'=>$uid]);
			 if ($res){
				...
				$title = $ll['title'];
				$cat_url = 'http://'.$_SERVER['HTTP_HOST'].url('cat',array('id'=>$id));
...
//								$ans_nlist[$ko1]['reply'] = $this->find_reply($vo1['reply'],$option);
								...
								if($ll['add_type']=='4'){
									$a = null;
									if(strstr($vo1['reply'],'@#')){
										$reply_arr = explode('@#',$vo1['reply']);
										foreach($reply_arr as $ko2=>$vo2){
											$reply = $this->find_reply($vo2,$option);
											foreach($option as $ko3=>$vo3){
												if($vo3['option']==$reply){
													$a .= $vo3['con'].'&';
												}
											}
										...
										$a = rtrim($a,'&');
										$a = str_replace("&","和",$a);
										$ans_nlist[$ko1]['reply'] = $a;
									}else{
										$reply = $this->find_reply($vo1['reply'],$option);
										foreach($option as $ko2=>$vo2){
											if($vo2['option']==$reply){
												$ans_nlist[$ko1]['reply'] = $vo2['con'];
											}
										}
									}
								}else{
									if(strstr($vo1['reply'],'@#')){
										...
										$ans_nlist[$ko1]['reply'] = implode('和',$reply);
									}else{
										$ans_nlist[$ko1]['reply'] = $this->find_reply($vo1['reply'],$option);
									}
...
//				array_push($content,$result['showapi_res_body']['pagebean']['contentlist']);
	...
	//餐饮娱乐选项内容后的搜索的ajax
	public function cyyl_search_ajax(){
		$keyword = $_POST['keyword'];
		$baidu = new baidu_api();
		//数量
		$page_size = 5;
		//页数
		$page_num = input('param.page_num');
		if($page_num){
		...
		$res = $baidu->place_region($keyword,'美食,休闲娱乐','北京','json',$page_size,$page_num);
		$arr['res'] = json_decode($res,true);
		return $arr;
	}

	//餐饮娱乐--具体搜索的详细信息的ajax
	public function cyyl_con_ajax(){
		$con_url = $_POST['con_url'];
		if(!empty($con_url)){
			$str = $this->phantomjs($con_url);
			$res = $this->get_con($str);
			return $res;
		}else{
			return 'error';
		}
...
//		dump($evaluate_arr);
	...
	public function cyyl_search_detail_ajax(){
		$id = $_POST['id'];
		$baidu = new baidu_api();
		if ($id){
			$result = $baidu->place_detail($id);
